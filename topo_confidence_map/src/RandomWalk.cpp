...
void TopologyMap::InitializeGridMap(const pcl::PointXYZ & oRobotPos) {
    ...
    m_oNodeGoal.x = oRobotPos.x;
    m_oNodeGoal.y = oRobotPos.y;
    m_oNodeGoal.z = oRobotPos.z;
...
void TopologyMap::HandleTrajectory(const nav_msgs::Odometry & oTrajectory) {
	...
	if (!(m_iTrajFrameNum % m_iOdomSampingNum)) {
        ...
        std::cout<<"fTouchNodeGoal: "<<fTouchNodeGoal<<std::endl;
        std::cout<<"robot.x: "<<m_vOdomShocks.back().x<<"robot.y: "<<m_vOdomShocks.back().y<<std::endl;
        //if it arrivals at the target node
        if(fTouchNodeGoal){

            //define candidate variables
	        std::vector<int> vMinCandidates;
	        std::vector<bool> vMinCandidStatus(m_vConfidenceMap.size(),true);
			...
			//if it is a reachable ground grid (some initial reachable grids are not the ground grids)
                    if(m_vConfidenceMap[i].travelable == 1 &&
		                m_vConfidenceMap[i].label == 2){

                        //if it is small
			            if (m_vConfidenceMap[i].travelTerm < 0.7){
		                    //get this grid into candidate vector
				            vMinCandidates.push_back(i);
			            }else{ 
			            	//
			            	if(!m_vConfidenceMap[i].coverNodeTimes)
			            	   m_vConfidenceMap[i].coverNodeTimes = m_iNodeTimes;
			            }//end else
		            }//end if
		        }//end for i
            ...
            if(vMinCandidates.size()){

	    	    std::vector<int> vGoalIdxs = Confidence::GetRandom(vMinCandidates.size(), 1);

	    	    ExtendedGM::OneDIdxtoPoint(m_oNodeGoal, vMinCandidates[vGoalIdxs[0]], m_oGMer.m_oFeatureMap);
            ...
            std::cout<<"******m_iNodeTimes: "<<m_iNodeTimes<<std::endl;
		...
		if(m_iNodeTimes > 0){
            ...
            std::cout << "goal.x: " << oRealGoalPoint.x << std::endl;
            oRealGoalPoint.y = m_oNodeGoal.y;
            std::cout << "goal.y: " << oRealGoalPoint.x << std::endl;
...
void TopologyMap::ComputeConfidence(const pcl::PointXYZ & oCurrRobotPos) {
    ...
    //compute distance term
    std::vector<int> vWalkGrndGrdIdxs;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pWalkGrndClouds(new pcl::PointCloud<pcl::PointXYZ>);
    for(int i=0;i!=vNearByIdxs.size();++i){
        int iNearGridId = vNearByIdxs[i].iOneIdx;
        vWalkGrndGrdIdxs.push_back(iNearGridId);
        pWalkGrndClouds->points.push_back(m_vConfidenceMap[iNearGridId].oCenterPoint);
	...
	for (int i = 0; i != m_oGMer.m_oFeatureMap.getSize()(0); ++i) {//i
		...
		for (int j = 0; j != m_oGMer.m_oFeatureMap.getSize()(1); ++j) {//j
			...
			if(m_vConfidenceMap[iGridIdx].label == 2){
		    	...
		    	gridMapData5(i, j) = m_vConfidenceMap[iGridIdx].coverNodeTimes;//.totalValue
